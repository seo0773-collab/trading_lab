...
def run_backtest(
    ...
    for t in range(start, n - 1):
        ...
        entry_signal = signal
        if entry_signal != 0 and cfg.edge_mult > 0 and ee is not None:
            if not (np.isfinite(ee[t]) and ee[t] >= cfg.edge_mult * 2 * fee):
                entry_signal = 0

        if pos == 0:
            if entry_signal != 0:
                pos, entry_px, entry_t = entry_signal, close[t], t
                bar_ret[t] -= fee  # 진입 비용
        else:
            # 봉별 수익 귀속
            ...
            if held >= cfg.horizon or opposite or t == n - 2:
                ...
                bar_ret[t] -= fee  # 청산 비용
                pos = 0
                # 조기 청산 직후 edge 조건까지 통과한 반대 방향만 재진입
...
def build_signals(df: pd.DataFrame, horizon: int):
    ...
    mult_ret = df[f"mhat_{horizon}"] / df["mult_close"] - 1.0
    price_ret = df[f"price_mid_{horizon}"] / df["close"] - 1.0
    cycle_ret = price_ret - mult_ret

    mult_dir = np.sign(mult_ret)
    mult_conf = (pup - 0.5).abs()
    price_dir = np.sign(price_ret)
    price_conf = price_ret.abs()
    aligned_dir = mult_dir.where(mult_dir == price_dir, 0.0)
    ...
    signals = {
        "mult_dir": mult_dir,
        "mult_conf": mult_conf,
        "mult_edge": mult_ret.abs(),
        "price_dir": price_dir,
        "price_conf": price_conf,
        "price_edge": price_ret.abs(),
        "aligned_dir": aligned_dir,
        "mult_ret": mult_ret,
        "price_ret": price_ret,
        "cycle_ret": cycle_ret,
    }
    return signals, (ou_dir, ou_conf)


def diagnostic_signal_mask(direction: pd.Series, conf: pd.Series, q: float, cfg: BTConfig) -> pd.Series:
    thr = rolling_conf_threshold(conf, q, cfg.quantile_window)
    start = max(int(len(conf) * cfg.ident_frac), cfg.quantile_window // 2)
    mask = ((np.arange(len(conf)) >= start) & thr.notna()
            & conf.notna() & (conf >= thr) & (direction != 0))
    if cfg.long_only:
        mask &= direction > 0
    return pd.Series(mask, index=conf.index)


def fmt_signal_diag(label: str, mask: pd.Series, direction: pd.Series,
                    future_ret: pd.Series) -> str:
    valid = mask & direction.notna() & future_ret.notna() & (direction != 0)
    n = int(valid.sum())
    if not n:
        return f"  {label:<18} n=0"
    signed = direction[valid] * future_ret[valid]
    return (f"  {label:<18} n={n:>5}  적중 {(signed > 0).mean():>6.1%}  "
            f"평균 gross {signed.mean() * 1e4:>+8.1f}bp  "
            f"중앙 {signed.median() * 1e4:>+8.1f}bp")
...
def main():
    ...
    args = ap.parse_args()
    if args.bars_per_year <= 0:
        ap.error("--bars-per-year는 0보다 커야 합니다")
    ...
    need = ["close", "mult_close", "m_slow", f"pup_{args.horizon}",
            f"mhat_{args.horizon}", f"price_mid_{args.horizon}"]
    ...
    sig, (ou_dir, ou_conf) = build_signals(df, args.horizon)
    kal_dir, kal_conf, kal_edge = sig["mult_dir"], sig["mult_conf"], sig["mult_edge"]
    ...
    for q in (0.70, 0.80, 0.85, 0.90, 0.95):
        ...
        results[q] = (rk, ro)
        print("  " + fmt_row(rk))
        long_hit = f"{rk.long_hit:.1%}" if rk.long_n else "n/a"
        short_hit = f"{rk.short_hit:.1%}" if rk.short_n else "n/a"
        print(f"{'':24}└ 롱 {rk.long_n}건 적중 {long_hit} / "
              f"숏 {rk.short_n}건 적중 {short_hit}")
        print("  " + fmt_row(ro))
        print()
    ...
    signal_thr = rolling_conf_threshold(kal_conf, 0.85, BTConfig.quantile_window)
    start = max(int(len(df) * BTConfig.ident_frac), BTConfig.quantile_window // 2)
    signal_mask = ((np.arange(len(df)) >= start) & signal_thr.notna()
                   & (kal_conf >= signal_thr) & (kal_dir != 0))
    if args.long_only:
        signal_mask &= kal_dir > 0
    eq = kal_edge[signal_mask].dropna() * 1e4
    print(f"    신호구간 기대수익 분포(bp, n={len(eq)}): "
          f"25% {eq.quantile(.25):.0f} / 중앙 {eq.quantile(.5):.0f} / "
          f"75% {eq.quantile(.75):.0f} / 95% {eq.quantile(.95):.0f}   "
          f"왕복비용 {rt_cost:.0f}bp")
    best_edge = None
    for em in (0.0, 1.0, 2.0, 3.0, 5.0, 8.0):
        ...
        print("  " + fmt_row(r))
        if em > 0 and r.n_trades >= 20 and (
                best_edge is None or r.sharpe > best_edge.sharpe):
            best_edge = r

    # --- 5) 배수 공간 vs 가격 공간 및 cycle slope 충돌 진단 ---
    print("\n[5] MULT vs PRICE 방향 및 cycle slope 진단 (q=0.85)")
    base_cfg = BTConfig(horizon=args.horizon, fee_bps=args.fee_bps,
                        conf_quantile=0.85, long_only=args.long_only,
                        exit_on_opposite=not args.no_early_exit)
    mult_mask = diagnostic_signal_mask(sig["mult_dir"], sig["mult_conf"], 0.85, base_cfg)
    agree = sig["mult_dir"] == sig["price_dir"]
    conflict = (sig["mult_dir"] != 0) & (sig["price_dir"] != 0) & ~agree
    valid_mult = mult_mask & sig["price_dir"].notna()
    agree_rate = float(agree[valid_mult].mean()) if valid_mult.any() else np.nan
    print(f"    MULT 신호 중 PRICE 방향 일치 {agree_rate:.1%} / 충돌 {1 - agree_rate:.1%}")

    for label, values in (("MULT", sig["mult_ret"]),
                          ("CYCLE contribution", sig["cycle_ret"]),
                          ("PRICE", sig["price_ret"])):
        bp = values[mult_mask].dropna() * 1e4
        print(f"    {label:<18} 예측(bp): 중앙 {bp.median():>+7.0f} / "
              f"25% {bp.quantile(.25):>+7.0f} / 75% {bp.quantile(.75):>+7.0f}")

    future_ret = df["close"].shift(-args.horizon) / df["close"] - 1.0
    print(fmt_signal_diag("MULT 전체", mult_mask, sig["mult_dir"], future_ret))
    print(fmt_signal_diag("방향 일치", mult_mask & agree, sig["mult_dir"], future_ret))
    print(fmt_signal_diag("방향 충돌", mult_mask & conflict, sig["mult_dir"], future_ret))
    print(fmt_signal_diag("충돌시 PRICE", mult_mask & conflict, sig["price_dir"], future_ret))

    cycle_abs_thr = sig["cycle_ret"].abs().rolling(
        base_cfg.quantile_window, min_periods=base_cfg.quantile_window // 2
    ).quantile(0.5).shift(1)
    weak_cycle = sig["cycle_ret"].abs() <= cycle_abs_thr
    cycle_dir = np.sign(sig["cycle_ret"])
    trend_aligned = (sig["mult_dir"] == cycle_dir) & (cycle_dir != 0)
    variants = [
        ("MULT", sig["mult_dir"], sig["mult_conf"], sig["mult_edge"]),
        ("PRICE", sig["price_dir"], sig["price_conf"], sig["price_edge"]),
        ("CYCLE", cycle_dir, sig["cycle_ret"].abs(), sig["cycle_ret"].abs()),
        ("ALIGNED", sig["aligned_dir"], sig["mult_conf"], sig["price_edge"]),
        ("CYCLE-WEAK", sig["mult_dir"].where(weak_cycle, 0.0),
         sig["mult_conf"], sig["price_edge"]),
        ("TREND-ALIGNED", sig["mult_dir"].where(trend_aligned, 0.0),
         sig["mult_conf"], sig["price_edge"]),
    ]
    diagnostic_results = {}
    print("\n    전략 비교 (동일 q=0.85)")
    for label, direction, conf, edge in variants:
        r = run_backtest(df, direction, conf, base_cfg, label, edge)
        diagnostic_results[label] = r
        print("  " + fmt_row(r))
        long_hit = f"{r.long_hit:.1%}" if r.long_n else "n/a"
        short_hit = f"{r.short_hit:.1%}" if r.short_n else "n/a"
        print(f"{'':24}└ 롱 {r.long_n}건 적중 {long_hit} / "
              f"숏 {r.short_n}건 적중 {short_hit}")

    print("\n[6] PRICE 기대수익 필터 (PRICE, q=0.85)")
    best_price_edge = None
    for em in (0.0, 5.0, 10.0, 15.0, 20.0, 30.0):
        cfg = BTConfig(horizon=args.horizon, fee_bps=args.fee_bps,
                       conf_quantile=0.85, long_only=args.long_only,
                       exit_on_opposite=not args.no_early_exit, edge_mult=em)
        r = run_backtest(df, sig["price_dir"], sig["price_conf"], cfg,
                         f"PRICE λ={em:>4.0f}", sig["price_edge"])
        print("  " + fmt_row(r))
        if em > 0 and r.n_trades >= 20 and (
                best_price_edge is None or r.sharpe > best_price_edge.sharpe):
            best_price_edge = r
    ...
    if best_edge is not None:
        ax.plot(best_edge.equity.index, best_edge.equity,
                label=f"KALMAN +edge filter ({best_edge.name.strip()})", lw=1.2,
                color="tab:green", alpha=0.7)
    for label, color in (("PRICE", "tab:orange"), ("ALIGNED", "tab:purple")):
        r = diagnostic_results[label]
        ax.plot(r.equity.index, r.equity, label=label, lw=1.4, color=color)
    if best_price_edge is not None:
        ax.plot(best_price_edge.equity.index, best_price_edge.equity,
                label=best_price_edge.name.strip(), lw=1.4, color="tab:brown")
    ...
    out_png = ROOT / "reports" / f"{name}_strategy_h{args.horizon}_fee{args.fee_bps:g}bp.png"
