...
`equity` 컬럼 하나를 가진 프레임. **초기값 1.0 기준 정규화** 곡선이어야 하며, 대시보드가 초기 자본을 곱해 계좌 금액·낙폭·실행 비교를 그립니다. 결과 화면에서는 `forecast.close`를 같은 초기 자본으로 정규화한 Buy & Hold 곡선과 누적 수익률도 함께 표시합니다.
