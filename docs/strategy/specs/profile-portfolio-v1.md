...
파라미터 스윕(`reports/profile_sizing/portfolio_sweep.md`, top_k×리밸런스×floor,
공정 벤치마크=EW 지수)을 **validation에서 선정 → test(holdout)에서 확인**(과적합 점검)한
결과, 검증 1위는 top_k=20·monthly·floor=1.0이며 holdout에서도 견고했다(val Sharpe−B&H
+0.055 → test +0.009, B&H 위 유지). 이를 기본 config로 채택. 패턴: 분산↑(top_k=20)·
추종↑(floor=1.0)일수록 위험조정 우위, floor=0/집중(top_k=5)은 하위권.
...
| --- | ---: | ---: | ---: |
| 우량주 30 | 1.140 | 1.045 | +0.095 |
| 확장 54(부진 포함) | 1.123 | 0.979 | **+0.144** |
...
### 재점검 2차 정정 (2026-06-16) — 벤치마크 cash-drag 편향

1차에서 "진짜 buy&hold"로 바꾼 것은 **over-correction**이었다. 유니버스가 1962년
시작이고 그때 30종목 중 10종목만 상장(2015년에야 30종목 완성)이라, 진짜 buy&hold는
미상장 종목 몫(초기 ~67%)을 수십 년 현금으로 묶어 수익이 부당하게 낮아진다
(cash-drag). 반면 전략은 가용 종목에 자본을 자유롭게 싣는다 → 비교가 전략에 유리하게
왜곡. 따라서 **주 벤치마크를 상시 완전투자하는 EW 지수**(실제 RSP 류, 공정)로 되돌리고,
진짜 buy&hold는 cash-drag 주의와 함께 참고로만 병기한다.

| phase=all | 전략 | EW 지수(주·공정) | 진짜 BH(cash-drag) |
| --- | ---: | ---: | ---: |
| CAGR | 13.9% | **18.0%** | 12.3% |
| MDD | **-29.0%** | -53.5% | -53.2% |
| Sharpe | **1.133** | 1.046 | 0.821 |

**확정 결론(공정 기준)**: 전략은 **수익률(CAGR)로는 EW 지수에 못 미치지만(13.9 vs 18%),
위험조정(Sharpe 1.13 vs 1.05)은 소폭 우위이고 MDD는 절반(-29 vs -54%)**이다. 즉 "수익은
양보하되 위험 대비 효율은 동률~소폭 우위, 낙폭은 크게 개선"이 정직한 결론이다.
(체결 무누수 수정의 영향은 미미해 결과 견고성은 유지.)
